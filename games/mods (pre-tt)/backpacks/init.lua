...
local function register_wool_backpack(colour,colourname)
	minetest.register_node("backpacks:backpack_wool_"..colour, {
        ...
        after_place_node = function(pos, placer, itemstack, pointed_thing)
            if not placer or not placer:is_player() or not placer:get_player_control().sneak then
                minetest.remove_node(pos)
                return itemstack
            else
                backpacks.after_place_node(pos, placer, itemstack, pointed_thing)
            end
        end,

        on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
            if clicker and not clicker:get_player_control().sneak then
                minetest.show_formspec(clicker:get_player_name(), "backpacks:backpack", backpacks.form)
            end
        end,
