...
if enabled then
    ...
    minetest.register_on_player_receive_fields(function(player, formname, fields)
        if formname == "" and fields.sneak then
            -- Toggle health bar on sneak tap
            local player_name = player:get_player_name()
            local player_entity = minetest.get_player_by_name(player_name)
            local gauge_entity = minetest.get_entity(player_name .. "'s_gauge_entity_name") -- Replace with your actual gauge entity name

            if gauge_entity then
                gauge_entity:remove()
            else
                add_gauge(player_entity)
            end
        end
    end)