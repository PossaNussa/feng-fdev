...
function petz.hq_attack(self, prty, tgtobj)
    local func = function()
        ...
        -- Proceed if queue is empty and creature is on the ground
        if kitz.is_queue_empty_low(self) then
            ...
            if dist <= 3 then
                ...
                -- Check if target has a Lua entity and height property
                local lua_entity = tgtobj:get_luaentity()
                if not lua_entity or not lua_entity.height then
                    return
                end
                
                local t_height = lua_entity.height
                ...
                if tpos.y + height > pos.y then
                    ...
                    local shift = {x = math.random() - 0.5, z = math.random() - 0.5}
                    kitz.lq_dumbwalk(self, kitz.pos_shift(tpos, shift))
                end
            else
                return true -- End function if target is out of range
            end
        end
    end
    kitz.queue_high(self, func, prty)
