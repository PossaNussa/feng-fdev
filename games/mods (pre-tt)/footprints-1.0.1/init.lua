...
if default_modpath then
	...
	footprints.register_trample_node("default:dirt", {
		...
		footprints.register_trample_node("default:mud", {
		trampled_node_name = "footprints:mud",
		trampled_node_def_override = {description = S("Mud with Footprint"),},
		hard_pack_node_name = "footprints:trail",
		footprint_opacity = 96,
		hard_pack_probability = HARDPACK_PROBABILITY,
		hard_pack_count = HARDPACK_COUNT,
	})
		footprints.register_trample_node("livingdesert:coldsteppe_ground", {
		trampled_node_name = "footprints:coldsteppe_ground",
		trampled_node_def_override = {description = S("Coldsteppe Ground with Footprint"),},
		hard_pack_node_name = "footprints:trail",
		footprint_opacity = 96,
		hard_pack_probability = HARDPACK_PROBABILITY,
		hard_pack_count = HARDPACK_COUNT,
		...
		footprints.register_trample_node("default:wet_sand", {
		trampled_node_name = "footprints:wet_sand",
		trampled_node_def_override = {description = S("Wet Sand with Footprint"),},
		hard_pack_node_name = "footprints:trail",
		footprint_opacity = 96,
		hard_pack_probability = HARDPACK_PROBABILITY,
		hard_pack_count = HARDPACK_COUNT,
	})
		footprints.register_trample_node("livingdesert:coldsteppe_ground2", {
		trampled_node_name = "footprints:coldsteppe_ground2",
		trampled_node_def_override = {description = S("Coldsteppe Ground with Footprint"),},
		hard_pack_node_name = "footprints:trail",
		footprint_opacity = 96,
		hard_pack_probability = HARDPACK_PROBABILITY,
...
local speed_modifier = 0.66 -- 50% speed when walking on mud
local lmud = 0.5

local function check_player_on_mud()
    for _, player in ipairs(minetest.get_connected_players()) do
        local pos = player:get_pos()
        pos.y = pos.y - 1 -- get the node player is standing on
        
        local node = minetest.get_node(pos).name
        if node == "footprints:mud" or node == "default:mud" then
            player:set_physics_override({speed = speed_modifier})
        elseif node == "default:liquid_mud_source" or node == "default:liquid_mud_flowing" then
            player:set_physics_override({speed = lmud})
        else
            player:set_physics_override({speed = 1}) -- reset to normal speed when not on mud
        end
    end
    -- Schedule the next check
    minetest.after(2.1, check_player_on_mud) -- adjust the time as needed
end
