...
-- Register MegaSlime mob with Creatura
local megamob = "livingslimes:" .. tname .. "_mega"
creatura.register_mob(megamob,{
    -- Engine properties
    infotext = name .. " Megaslime",
    visual_size = { x = def.size * 20, y = def.size * 20 },
    visual = "mesh",
    mesh = def.aquatic and "slime_liquid.b3d" or "slime_land.b3d",
    textures = {
        {"livingslimes_slime_block.png^[colorize:" .. def.color,"livingslimes_slime_block.png^[colorize:" .. def.color},
    ...
    glow = def.glow,
    -- Creatura properties
    max_health = math.random(80, 1000),
		armor_groups = (function()
			local groups = {fleshy = 100, fire = 100}
			local additional_groups = def.armor_groups or {}
			for group,value in pairs(additional_groups) do
    ...
    hitbox = {
        width = def.size * 20 / 10,
        height = def.size * 20 / 5,
    },		animations = {
			none = {
				range = { x = 0, y = 0 },
				speed = 10,
	...
	-- Create spawning rules for slime mob
	creatura.register_abm_spawn(megamob,{
		chance = 200000,
		interval = 3000,
		min_height = def.min_height,
		max_height = def.max_height,
