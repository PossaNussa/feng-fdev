...
local function apply_illumination(player)
    local light_node = get_light_node(player)
    if light_node then
        local pos = find_light_pos(player:get_pos())
        if pos then
            minetest.set_node(pos, {name = light_node})
        end
    end
end

offhand.register_on_item_change(function(player, _, _)
    apply_illumination(player)
end)