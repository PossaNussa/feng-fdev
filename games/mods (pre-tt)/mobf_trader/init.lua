...
mobf_trader.trader_entity_on_activate = function(self, staticdata, dtime_s)
    ...
    if staticdata then
        ...
        if not self.trader_animation or self.trader_does then
            self.trader_animation = self.trader_does or "default_animation"  -- Set a default animation if it's nil
        end
        ...
        if self.animation[self.trader_animation..'_START'] and self.animation[self.trader_animation..'_END'] then
            self.object:set_animation({x=self.animation[self.trader_animation..'_START'], y=self.animation[self.trader_animation..'_END']}, self.animation_speed-5+math.random(10))
        else
            if self.animation['stand_START'] and self.animation['stand_END'] then
                self.object:set_animation({x=self.animation['stand_START'], y=self.animation['stand_END']}, self.animation_speed-5+math.random(10))
            end
        end
	...
	if not self.trader_name or self.trader_name == '' or self.trader_id == '' then
		-- no name supplied - it will be chosen automatically
		-- the type of trader is unknown at this stage
...
-- Define the Trader mob using mobs:register_mob
mobs:register_mob("mobf_trader:trader", {
    type = "npc",
    hp_max = 20,
    armor = 100,
    passive = true,
    ...
    view_range = 10,
    drops = {
        {name = "animalia:porkchop_raw", chance = 1, min = 0, max = 2},
    },
    water_damage = 0,
    lava_damage = 5,
    light_damage = 0,
    animation = {
	    ...
	    -- Define the trader's on_rightclick function
    on_rightclick = function(self, clicker)
        mobf_trader.trader_entity_trade(self, clicker)
    end,
})
...
-- Define the trader's on_rightclick function
function mobf_trader.on_rightclick(self, clicker)
    mobf_trader.trader_entity_trade(self, clicker)
end
