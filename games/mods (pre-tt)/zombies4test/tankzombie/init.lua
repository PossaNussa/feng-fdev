...
mobs:register_mob("tankzombie:tankzombie", {
	...
	drops = {
		...
		{name = "default:uranium_lump", chance = 2, min = 1, max = 1},
    ...
    local players = minetest.get_connected_players()
	    	...
	    	for _, player in ipairs(players) do
	    	   ...
	    	   -- self.object:set_animation({x=200, y=420},15,0, false)
	    	   
				minetest.sound_play("missozzy", {pos = pos, gain = 0.5})
					
				
	    	    player:set_pos({
	    			x=player:get_pos().x+5,
	    			y=player:get_pos().y+3,
	    		  z=player:get_pos().z
	    		})

	    		-- Causar dano no atack :
...
minetest.register_on_punchplayer(function(player, hitter, time_from_last_punch, tool_capabilities, dir)
    -- Check if the hitter is an object and has a Lua entity associated with it
    if hitter:is_player() == false and hitter:get_luaentity() then
        -- Check if the Lua entity is a tank zombie
        if hitter:get_luaentity().name == "tankzombie:tankzombie" then
            ...
            -- Play the sound
            minetest.sound_play("missozzy", { pos = player_pos, gain = 0.5 })
        end
    end
end)
