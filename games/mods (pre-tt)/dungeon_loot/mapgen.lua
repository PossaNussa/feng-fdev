...
local function MathRandomWrapper()
    return {
...
-- Register the urn node
minetest.register_node("dungeon_loot:urn", {
    description = "Burial Urn",
    drawtype = "mesh",
    mesh = "urn.obj",
    tiles = {"urn.png"},
    groups = {cracky = 3},
...
on_construct = function(pos)
    ...
    local max_items = dungeon_loot.STACKS_PER_CHEST_MAX -- Assumes this variable is accessible
    inv:set_size("main", max_items)
    populate_chest(pos, MathRandomWrapper(), "normal")
end,
    on_destruct = function(pos)
        ...
        local to_remove = math.floor(size / 2)
        ...
        for i = 1, size do
            ...
            if not stack:is_empty() then
                minetest.add_item(above_pos, stack)
            end
        end
    end,
})

-- Register the urn to spawn in caves
minetest.register_decoration({
    name = "dungeon_loot:urn_cave_spawn",
    deco_type = "simple",
    place_on = {"default:stone"},
    sidelen = 16,
    fill_ratio = 0.01,  -- Adjust to your needs
    biomes = {"underground"},
    decoration = "dungeon_loot:urn",
    height_max = -50,
    height_min = -31000,
    spawn_by = "air",
    num_spawn_by = 1,
