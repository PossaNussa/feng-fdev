...
local function allow_metadata_inventory_put (pos, listname, index, stack, player)
	...
	if listname == "bucket_out" then
		if stack:get_name():match("^bucket:bucket_empty_uni[_%a]*$") and not fluidity.florbs.get_is_florb(stack) then
			return 0
		end
	...
	if listname == "coolant" then
		if stack:get_name():match("^bucket:bucket_water_uni[_%a]*$") then
			return 1
		end
	end
...
local function caster_node_timer(pos, elapsed)
	...
	if inv:get_stack("coolant", 1):get_name():match("^bucket:bucket_water_uni[_%a]*$") then
	...
	if (bucket_name:find("bucket") and bucket_name ~= "bucket:bucket_empty_uni[_%a]*$") or (not fluidity.florbs.get_is_empty_florb(bucket_in) and
	...
	if (bucket_name == "bucket:bucket_empty_uni[_%a]*$" or fluidity.florbs.get_is_florb(bucket_out)) and metal and metal.fluid ~= "" and bucket_out:get_count() == 1 then
...
if minetest.get_modpath("pipeworks") ~= nil then
	...
	local function insert_object(pos, node, stack, direction, owner)
		...
		if stack_name == "bucket:bucket_empty_uni[_%a]*$" or fluidity.florbs.get_is_empty_florb(stack) then
			return inv:add_item("bucket_out", stack)
		elseif stack_name:match("^bucket:bucket_water_uni[_%a]*$") then
