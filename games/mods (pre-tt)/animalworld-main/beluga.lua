...
on_spawn = function(self, staticdata)
    ...
    local initial_size_factor = math.random(0.5, 5.0)
    self.object:set_properties({visual_size = {x=initial_size_factor, y=initial_size_factor}})
    -- Adjust collision box to match size
    local scale = initial_size_factor
    self.object:set_properties({
        collisionbox = {
            -0.8 * scale, -0.01 * scale, -0.8 * scale,
            0.8 * scale, 1.2 * scale, 0.8 * scale
        }
    })
