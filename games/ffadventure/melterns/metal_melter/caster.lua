...
if listname == "coolant" then
    if not stack:get_name():find("^bucket:bucket_water[_%a]*$") then
        return 0
    end
end
...
if inv:get_stack("coolant", 1):get_name():find("^bucket:bucket_water[_%a]*$") then
    ...
    -- Handle input bucket or florb
    local input = inv:get_stack("bucket_in", 1)
    local input_name = input:get_name()
    
    -- Check if the input is a florb
    if fluidity.florbs.get_is_florb(input) then
        local contents, fluid_name, capacity = fluidity.florbs.get_florb_contents(input)
        local fluid_metal = fluidity.get_metal_for_fluid(fluid_name)
        
        if fluid_metal then
            local take = math.min(contents, metal_caster.max_metal - metal.amount)
            
            if take > 0 then
                -- Take fluid from the florb
                local new_florb, amount_taken = fluidity.florbs.take_fluid(input, take)
                
                -- Update metal in the caster
                metal.fluid = fluid_name
                metal.amount = metal.amount + amount_taken
                
                -- Update the florb in the inventory
                inv:set_stack("bucket_in", 1, new_florb)
                
                refresh = true
            end
        end
    elseif input_name:find("bucket") and input_name ~= "bucket:bucket_empty" then
        -- Handle regular buckets
        local bucket_fluid = bucket.get_liquid_for_bucket(input_name)
        local fluid_is_metal = fluidity.get_metal_for_fluid(bucket_fluid) ~= nil
        
        if fluid_is_metal then
            if metal.fluid == "" or metal.fluid == bucket_fluid then
                if metal.amount + 1000 <= metal_caster.max_metal then
                    metal.amount = metal.amount + 1000
                    metal.fluid = bucket_fluid
                    inv:set_stack("bucket_in", 1, ItemStack("bucket:bucket_empty"))
                    refresh = true
                end
            end
        end
...
if minetest.get_modpath("pipeworks") ~= nil then
	...
	local function insert_object(pos, node, stack, direction, owner)
		...
		elseif stack_name == "bucket:bucket_water[_%a]*$" then
...
minetest.register_node("metal_melter:metal_caster", {
	...
	fluid_buffers = {
		...
		metal = {
			...
			accepts  = "group:molten_metal", "fluidity:florb_molten_tin", "fluidity:florb:molten_iron",
		}
	},
...
metal_caster.register_cast("iron", {
    description = "Iron Bar",
    result = "default:ironbar", -- Replace with your ingot item
    cost = 144,
    typenames = {"ingot"}
})