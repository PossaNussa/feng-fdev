...
local function allow_metadata_inventory_put (pos, listname, index, stack, player)
	...
	if listname == "bucket_out" then
		if stack:get_name() ~= "bucket:bucket_empty[_%a]*$" and not fluidity.florbs.get_is_florb(stack) then
			return 0
		end
	...
	if listname == "heat" then
		...
		if not string.match(stack_name, "^bucket:bucket_lava_uni[_%a]*$") then
			return 0
		end
	end
...
if string.match(inv:get_stack("heat", 1):get_name(), "^bucket:bucket_lava[_%a]*$") then
	...
	if (bucket_name:find("bucket") and bucket_name ~= "bucket:bucket_empty[_%a]*$") or (not fluidity.florbs.get_is_empty_florb(bucket_in) and
		...
		if is_florb then
			...
			if empty_bucket then
				inv:set_list("bucket_in", {"bucket:bucket_empty[_%a]*$"})
				refresh = true
			end
		end
	end
	...
	if (bucket_name == "bucket:bucket_empty[_%a]*$" or fluidity.florbs.get_is_florb(bucket_out)) and metal.fluid ~= "" and bucket_out:get_count() == 1 then
...
if minetest.get_modpath("pipeworks") ~= nil then
	...
	local function insert_object(pos, node, stack, direction, owner)
		...
		if stack_name == "bucket:bucket_empty[_%a]*$" or fluidity.florbs.get_is_empty_florb(stack) then
			return inv:add_item("bucket_out", stack)
		elseif stack_name == "bucket:bucket_lava[_%a]*$" then
