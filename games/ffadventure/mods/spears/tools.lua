...
if not DISABLE_STONE_SPEAR then
	spears_register_spear('flint', 'Flint', 1.1, 5, 'default:flint')
	spears_register_spear('wood', 'Wood', 2.5, 10, 'group:wood')
	spears_register_spear('bone', 'Bone', 3.2, 14, 'bonemeal:bone')
	spears_register_spear('stone', 'Stone', 4, 20, 'group:stone')
		spears_register_spear('iron', 'Iron', 5.5, 30, 'default:ironbar')
...
if not DISABLE_IRON_SPEAR then
	...
	if not DISABLE_STEEL_SPEAR then
		...
		spears_register_spear('csteel', 'Cobalt Steel', 7.5, 45, 'default:cobaltsteelbar')
	end
	...
	if not DISABLE_BRONZE_SPEAR then
		...
		spears_register_spear('iron', 'Iron', 5.7, 35, 'default:ironbar')
	end
else
	if not DISABLE_STEEL_SPEAR then
...
if not DISABLE_DIAMOND_SPEAR then
	spears_register_spear('diamond', 'Titanium', 8, 40, 'default:titaniumbar')
end
