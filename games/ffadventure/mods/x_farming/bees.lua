...
minetest.register_node('x_farming:bee_hive_saturated', {
    ...
    on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
        ...
        if stack_name == 'vessels:glass_bottle' or stack_name == 'x_farming:glass_bottle' then
            ...
            tick_hive(pos)
        elseif stack_name == 'default:machete' then
...
minetest.register_globalstep(function(dtime)
    ...
    local spawn_rate = 0.0005
    local spawnpos, liquidflag = x_farming.get_spawn_pos_abr(dtime * 10, 3, abr * 16, spawn_rate, spawn_reduction)  -- Multiply dtime by 10
    local tod = minetest.get_timeofday()
    local is_day = false
...
minetest.register_chatcommand("delete_bees", {
    description = "Delete all x_farming:bee nodes within 200 blocks",
    privs = {server = true},
    func = function(name)
        ...
        for x = minp.x, maxp.x do
            for y = minp.y, maxp.y do
                for z = minp.z, maxp.z do
                    ...
                    if node.name == "x_farming:bee" then
        ...
        return true, "Deleted " .. count .. " x_farming:bee nodes"
    end,
})
