...
-- Place inside the "default" mod.

local storage = minetest.get_mod_storage()

-- 20% orange tint (alpha ≈ 51/255)
local ORE_TEX     = "default_granite.png^(default_mineral_ruby.png^[colorize:#ff7f00:81)"
local REFINED_TEX = "ruby.png^[colorize:#ff7f00:81"

-- Items
minetest.register_craftitem("default:raw_kyawthuite", {
  description = "Raw Kyawthuite",
  inventory_image = "rkyaw.png",
})

minetest.register_craftitem("default:kyawthuite", {
  description = "Kyawthuite",
  inventory_image = REFINED_TEX,
})

-- Shapeless refine: any stone + raw -> refined
minetest.register_craft({
  type = "shapeless",
  output = "default:kyawthuite",
  recipe = {"group:stone", "default:raw_kyawthuite"},
})

-- Singular ore node
minetest.register_node("default:ore_kyawthuite", {
  description = "Kyawthuite Ore",
  tiles = { ORE_TEX },
  is_ground_content = true,
  groups = { cracky = 3, stone = 1 },
  drop = "default:raw_kyawthuite",
  sounds = default.node_sound_stone_defaults(),
})

-----------------------------------------------------------------------
-- Placement: exactly one ore in the entire map, inside default:granite
-----------------------------------------------------------------------
local placed = storage:get_string("kyaw_placed") == "true"

local function set_placed_at(pos)
  storage:set_string("kyaw_placed", "true")
  storage:set_string("kyaw_pos", minetest.pos_to_string(pos))
  placed = true
end
...
minetest.register_on_generated(function(minp, maxp, seed)
  ...
  local pos_list = minetest.find_nodes_in_area(minp, maxp, {"default:granite"})
  if not pos_list or #pos_list == 0 then return end

  -- Deterministic selection
   -- Deterministic selection (no PseudoRandom, no "Range too large")
  local seed_str = minetest.get_mapgen_setting("seed") or tostring(seed or 0)
  local seed_num = tonumber(seed_str)
  if not seed_num then
    local h = 0
    for i = 1, #seed_str do
      h = (h * 33 + seed_str:byte(i)) % 2147483647
    end
    seed_num = h
  end
  -- Mix in chunk coords so different mapchunks choose different granite nodes
  local mix = (
      (seed_num % 2147483647)
    + ((minp.x * 1103515245 + 12345) % 2147483647)
    + ((minp.z * 214013     + 2531011) % 2147483647)
    + ((minp.y * 134775813  + 1) % 2147483647)
  ) % 2147483647
  local idx = (mix % #pos_list) + 1
  local target = pos_list[idx]


  if minetest.get_node(target).name == "default:granite" then
    minetest.set_node(target, { name = "default:ore_kyawthuite" })
    set_placed_at(target)
  else
    for i = 1, #pos_list do
      local p = pos_list[i]
      if minetest.get_node(p).name == "default:granite" then
        minetest.set_node(p, { name = "default:ore_kyawthuite" })
        set_placed_at(p)
        break
      end
    end
...
minetest.register_chatcommand("whereis_kyaw", {
  description = "Show Kyawthuite ore position (if any).",
  privs = { server = true },
  func = function()
    if storage:get_string("kyaw_placed") == "true" then
      return true, "Kyawthuite ore at " .. storage:get_string("kyaw_pos")
    else
      return true, "Kyawthuite ore not yet placed."
    end
  end,
})
