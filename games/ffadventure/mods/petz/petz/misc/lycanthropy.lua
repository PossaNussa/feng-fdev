...
lycanthropy.werewolf.collisionbox = {-0.3, 0.0, -0.3, 0.3, 2.1, 0.3}
...
-- Registering aliases
minetest.register_alias("etc:dust", "etcetera:dust")
minetest.register_alias("default:sapling", "dynamic_trees:apple_tree_sapling")
minetest.register_alias("mobs:leather", "animalia:leather")
...
minetest.register_on_punchplayer(function(player, hitter, time_from_last_punch, tool_capabilities, dir, damage)
    ...
    -- Check if the entity is a wolf or werewolf
    if not (hitter_ent.name == "petz:wolf" or hitter_ent.name == "petz:werewolf") then
        return
    end
    
    local is_black_wolf = hitter_ent.name == "petz:wolf" and hitter_ent.texture_no == (#hitter_ent.skin_colors - hitter_ent.mutation + 1)
    local bit_by_wolf = hitter_ent.name == "animalia:wolf" and (math.random(1, petz.settings.lycanthropy_infection_chance_by_wolf) == 1)
    local bit_by_werewolf = hitter_ent.name == "petz:werewolf" and (math.random(1, petz.settings.lycanthropy_infection_chance_by_werewolf) == 1)
...
mobs:register_mob("petz:werewolf", {
    -- Basic properties
    type = "monster",
    passive = false,
    ...
    pathfinding = true,
    reach = 3,
    damage = 4, -- Adjust based on balance
    hp_min = 10,
    hp_max = 200, -- Adjust as needed
    armor = 70,
	lava_damage = 5,
	light_damage = 5,
	fall_damage = 2,
	water_damage = 2,
    visual = "mesh",
    mesh = "petz_werewolf.b3d",
    textures = {
        {"petz_werewolf_dark_gray.png"},
        {"petz_werewolf_gray.png"},
        {"petz_werewolf_brown.png"},
        {"petz_werewolf_white.png"},
    },
    makes_footstep_sound = true,
    sounds = {
        random = "werewolf_growl",
        attack = "werewolf_snarl",
	...
	drops = {
		{name = "animalia:leather", chance = 15, min = 1, max = 2},
		{name = "animalia:venison_raw", chance = 25, min = 1, max = 3},
 ...
 mobs:register_spawn("petz:werewolf", {"group:soil", "variety:meadow_dirt_with_grass"}, 15, 0, 48000, 3, 31000, false)
