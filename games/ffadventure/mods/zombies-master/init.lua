...
for name, def in pairs(minetest.registered_entities) do
    if string.match(name, "^zombies:") then
        ...
        def.on_step = function(self, dtime)
            -- Aggro scan every 0.5s; players only; 40-node cap
            self._aggro_timer = (self._aggro_timer or 0) + dtime
            if self._aggro_timer >= 0.5 then
                self._aggro_timer = 0
                local mypos = self.object:get_pos()
                local nearest, nd = nil, 40
                for _, pl in ipairs(get_players()) do
                    local d = vdist(mypos, pl:get_pos())
                    if d <= nd then nearest, nd = pl, d end
                end
                if nearest then
                    self.state = "attack"
                    self.attack = nearest
            ...
            -- Block breaking only during hordes (kept here; cheap neighbor checks)
            if horde_active then
                ...
                for _, target in ipairs(targets) do
                    ...
                    if name ~= "air" and name ~= "ignore" and not minetest.is_protected(target, "") then
                        local time
                        if minetest.get_item_group(name, "door") > 0 or name:find("^doors:") then
                            time = 5
                        elseif minetest.get_item_group(name, "crumbly") > 0 then
                            time = 5
                        elseif minetest.get_item_group(name, "tree") > 0 or minetest.get_item_group(name, "wood") > 0 then
                            time = 10
                        elseif minetest.get_item_group(name, "cracky") > 0 then
                            time = 20
                        end
                        if time then
...
local function set_random_uk_texture_on_spawn(self)
  ...
  mobs:register_spawn("zombies:axolotl", {"default:water_source"}, 15, 0, 95000, 5, 31000, nil)
  -- Spawn conditions for 'zombie' with reduced active count
...
-- === Persist zombie skins across unload/reload (no re-randomization) ===
for name, def in pairs(minetest.registered_entities) do
  if name:match("^zombies:") then
    local old_on_activate    = def.on_activate
    local old_get_staticdata = def.get_staticdata

    def.on_activate = function(self, staticdata, dtime_s)
      -- Read stored skin (if any)
      local stored
      if staticdata and staticdata ~= "" then
        local ok = minetest.deserialize(staticdata)
        if type(ok) == "table" and ok.zskin then
          stored = ok.zskin
        end
      end

      -- Let original initializer run first (in case it sets up textures)
      if old_on_activate then
        old_on_activate(self, staticdata, dtime_s)
      end

      if stored then
        -- Restore previously saved skin
        self._zskin = stored
        self.object:set_properties({ textures = self._zskin })
      else
        -- Capture whatever texture is currently set (do NOT choose a new one)
        local props = self.object.get_properties and self.object:get_properties() or nil
        local current = (props and props.textures)
                        or self.base_texture
                        or def.textures
        self._zskin = current
      end
    end
    ...
    def.get_staticdata = function(self)
      -- Merge with any existing staticdata the entity wants to save
      local out = { zskin = self._zskin }
      if old_get_staticdata then
        local s = old_get_staticdata(self)
...
-- zombies_awards.lua
-- Requires rubenwardy/awards. Safe to load after all entities are registered.

if not minetest.get_modpath("awards") then
  minetest.log("warning", "[zombies_awards] awards not installed; skipping")
  return
end

local unlock = awards.unlock -- API: awards.unlock(player_name, award_name)  (awards >= 3.x)
-- Ref: API shows register_achievement + unlock; HUD/chat shown on unlock.

--------------------------------------------------------------------------
-- 1) Register awards (titles / descriptions only; icons optional)
--------------------------------------------------------------------------

local A = awards.register_achievement

-- Counted slayer sets
A("z_award_slayer",           { title="Slayer",            description="Kill 500 zombies, crawlers, sea-zombies, draugr, or mummies!", difficulty=2, trigger={target=500} })
A("z_award_disarm_armless",   { title="Disarm the Armless",description="Kill 50 One-Arm Zombies!", difficulty=2, trigger={target=50} })
A("z_award_ham_fisted",       { title="A Ham-Fisted Attempt", description="Kill 50 Hamsterfolk!", difficulty=1, trigger={target=50} })
A("z_award_call_me_jack",     { title="Call Me Jack",      description="Kill 12 Cyclopes!", difficulty=3, trigger={target=12} })

-- Jungle hunters
A("z_award_on_the_prowl",     { title="On the Prowl",      description="Kill 20 lion- or cheetah-folk!", difficulty=2, trigger={target=20} })
A("z_award_king_of_jungle",   { title="King of the Jungle",description="Kill 50 lion- or cheetah-folk!", difficulty=3, trigger={target=50} })

-- Boss-skin specifics (kills)
A("z_award_mighty_conan",     { title="Mighty Conan",      description="Kill 8 Barbarians!", difficulty=3, trigger={target=8} })
A("z_award_trollkiller",      { title="Trollkiller",       description="Kill 5 Trolls!", difficulty=3, trigger={target=5} })
A("z_award_my_swamp_now",     { title="My Swamp Now",      description="Kill an Ogre!", difficulty=2, trigger={target=1} })
A("z_award_bone_be_gone",     { title="Bone-be-Gone",      description="Kill a Bone Golem!", difficulty=2, trigger={target=1} })

-- Boss-skin specifics (deaths)
A("z_award_savage_conan",     { title="Savage Conan",      description="Die to a Barbarian!", difficulty=3 })
A("z_award_oni_oh_no",        { title="Oni? Oh No!",       description="Die to an Oni!", difficulty=3 })


A("z_award_exotic_prey",      { title="Exotic Prey",        description="Pick up exotic meat!", difficulty=1 })
A("z_award_survivor",         { title="Survivor",           description="Clear a horde without dying", difficulty=3 })

A("z_award_wrong_field",      { title="In the Wrong Field", description="Kill a scarecrow zombie!", difficulty=2 })
A("z_award_baconator",        { title="Baconator",          description="Kill a pig zombie!", difficulty=2 })
A("z_award_apocalypse_when",  { title="Apocalypse When?",   description="Who did I just kill?", difficulty=2 })

A("z_award_beneath_rim",      { title="Beneath the Rim of the Sky", description="Kill a Draugr!", difficulty=2 })
A("z_award_drownin",          { title="Drownin'",           description="Kill a Sea Zombie!", difficulty=1 })
A("z_award_break_his_ankhles",{ title="Break His Ankhles!", description="Kill a Mummy!", difficulty=2 })
A("z_award_dead_once_more",   { title="Dead Once More",     description="Kill an Undead Knight!", difficulty=3 })
A("z_award_boss_fight",       { title="Boss Fight!",        description="Fight a Boss!", difficulty=3 })
A("z_award_one_eyed_menace",  { title="One-Eyed Menace",    description="Kill a Cyclops!", difficulty=3 })
A("z_award_jaxolaughter",     { title="Jaxolaughter",       description="Kill a Jaxolotl!", difficulty=3 })
A("z_award_ho_ho_horror",     { title="Ho-Ho-Horror",       description="Kill a Snow Golem!", difficulty=2 })
A("z_award_leviathan_slayer", { title="Leviathan Slayer",   description="Kill a Mosasaurus!", difficulty=3 })

A("z_award_mortal_enemy",     { title="Mortal Enemy",       description="Kill or die to a Frogman!", difficulty=2 })
A("z_award_knavish_gnolls",   { title="Knavish Gnolls",     description="Kill or die to a Gnoll!", difficulty=2 })
A("z_award_hamster_fail",     { title="Did I Just Die to a Hamster?", description="Die to a Hamsterfolk!", difficulty=1 })

A("z_award_bigfoot",          { title="I Swear I saw Him!",           description="Encounter a Big Ape!", difficulty=1 })
A("z_award_i_could_never",    { title="I Could Never Be Him", description="Encounter a Buff Pig!", difficulty=1 })
A("z_award_seaworthy_mate",   { title="A Seaworthy Mate",   description="Encounter a Parrot!", difficulty=1 })

A("z_award_golemancer",       { title="Golemancer",         description="Spawn a Golem!", difficulty=2 })


local ZA_store = minetest.get_mod_storage()
local ZA_counters = {}

local function za_load(p)
  if ZA_counters[p] then return ZA_counters[p] end
  local s = ZA_store:get_string("cnt:"..p)
  ZA_counters[p] = (s ~= "" and minetest.deserialize(s)) or {}
  return ZA_counters[p]
end

local function za_save(p)
  ZA_store:set_string("cnt:"..p, minetest.serialize(ZA_counters[p] or {}))
end

local function za_inc(p, key, n)
  local t = za_load(p); t[key] = (t[key] or 0) + (n or 1); za_save(p); return t[key]
end

local function za_get(p, key)
  return (ZA_counters[p] or za_load(p))[key] or 0
end


--------------------------------------------------------------------------
-- 2) Utility: read current textures safely (no re-randomization)
--------------------------------------------------------------------------

local function entity_textures(self, def)
  -- Prefer object properties (live), fall back to base/def
  local props = self.object and self.object.get_properties and self.object:get_properties()
  local t = (props and props.textures) or self._zskin or self.base_texture or (def and def.textures) or {}
  if type(t) == "string" then return { t } end
  if type(t[1]) == "table" then return t[1] end -- handle { { "tex.png", ... } }
  return t
end

local function any_tex_matches(texlist, patterns)
  for _, tex in ipairs(texlist) do
    for _, pat in ipairs(patterns) do
      if type(tex) == "string" and tex:find(pat) then
        return true
      end
    end
  end
...
local BASE_ZOMBIES = {
  ["zombies:zombie"]=true, ["zombies:normal"]=true,
  ["zombies:1arm"]=true,  ["zombies:crawler"]=true,
}

--------------------------------------------------------------------------
-- 3) Pick-up award: Exotic Prey (uses engine item_pickup callback)
--------------------------------------------------------------------------

minetest.register_on_item_pickup(function(itemstack, picker)
  if not (picker and picker:is_player()) then return end
  if itemstack and itemstack:get_name() == "animalia:venison_raw" then
    unlock(picker:get_player_name(), "z_award_exotic_prey")
  end
end) -- Uses core.register_on_item_pickup/item_pickup. :contentReference[oaicite:0]{index=0}
---------------------------------------------------------------------------
-- REPLACEMENT: Robust kill detection + progress triggers (drop-in)
-- - Uses Awards triggers so counted awards show/save progress in UI
-- - Wraps entity on_punch; no texture assignment; reads current textures only
---------------------------------------------------------------------------


-- === Awards helpers ===
local function ZA_unlock(name, id)
  if not name or not id then return end
  if awards.unlock then return awards.unlock(name, id) end
  if awards.give_achievement then return awards.give_achievement(name, id) end
end

-- Triggers so Awards shows progress bars in the Achievements tab
awards.register_trigger("kill_anyzombie", { type = "counted",  progress = "Zombie kills: @1/@2" })
awards.register_trigger("kill_onearm",    { type = "counted",  progress = "One-Arm kills: @1/@2" })
awards.register_trigger("kill_hamster",   { type = "counted",  progress = "Hamsterfolk kills: @1/@2" })
awards.register_trigger("kill_cyclops",   { type = "counted",  progress = "Cyclops kills: @1/@2" })
awards.register_trigger("kill_jungle",    { type = "counted",  progress = "Junglefolk kills: @1/@2" })
awards.register_trigger("kill_boss_bg1",  { type = "counted",  progress = "bg1 boss kills: @1/@2" })
awards.register_trigger("kill_boss_troll",{ type = "counted",  progress = "Troll bosses: @1/@2" })
awards.register_trigger("kill_boss_bg5",  { type = "counted",  progress = "bg5 boss kills: @1/@2" })
awards.register_trigger("kill_boss_bg9",  { type = "counted",  progress = "bg9 boss kills: @1/@2" })

-- Safe accessors to the notify functions (no-ops if absent)
local function safe_notify(f) return function(p) if f and p then f(p) end end end
local notify_anyzombie  = safe_notify(awards.notify_kill_anyzombie)
local notify_onearm     = safe_notify(awards.notify_kill_onearm)
local notify_hamster    = safe_notify(awards.notify_kill_hamster)
local notify_cyclops    = safe_notify(awards.notify_kill_cyclops)
local notify_jungle     = safe_notify(awards.notify_kill_jungle)
local notify_bg1        = safe_notify(awards.notify_kill_boss_bg1)
local notify_troll      = safe_notify(awards.notify_kill_boss_troll)
local notify_bg5        = safe_notify(awards.notify_kill_boss_bg5)
local notify_bg9        = safe_notify(awards.notify_kill_boss_bg9)

-- === Texture readers (no re-randomization) ===
local function entity_textures_from(self, def)
  local props = self.object and self.object.get_properties and self.object:get_properties()
  local t = (props and props.textures) or self._zskin or self.base_texture or (def and def.textures) or {}
  if type(t) == "string" then return { t } end
...
local function any_tex_matches(texlist, patterns)
  for _, tex in ipairs(texlist) do
    for _, pat in ipairs(patterns) do
      if type(tex) == "string" and tex:find(pat) then return true end
    end
  end
  return false
end
...
-- === Entity sets ===
local SLAYER_SET = {
  ["zombies:zombie"]=true, ["zombies:normal"]=true, ["zombies:1arm"]=true,
  ["zombies:crawler"]=true, ["zombies:seazombie"]=true, ["zombies:draugr"]=true,
  ["zombies:mummy"]=true,
}
local JUNGLE_SET = {
  ["zombies:lionman"]=true, ["zombies:cheetahman"]=true,
  ["zombies:cheetarcher"]=true, ["zombies:lion_warrior"]=true,
  ["zombies:cheetahwarrior"]=true,
}
local BASE_ZOMBIES = {
  ["zombies:zombie"]=true, ["zombies:normal"]=true,
  ["zombies:1arm"]=true,  ["zombies:crawler"]=true,
...
-- === One-off unlocks (non-counted) ===
local function unlock_one_offs(pname, en, captured_textures)
  if     en == "zombies:draugr"       then ZA_unlock(pname, "z_award_beneath_rim")
  elseif en == "zombies:seazombie"    then ZA_unlock(pname, "z_award_drownin")
  elseif en == "zombies:mummy"        then ZA_unlock(pname, "z_award_break_his_ankhles")
  elseif en == "zombies:undeadknight" then ZA_unlock(pname, "z_award_dead_once_more")
  elseif en == "zombies:badguy"       then ZA_unlock(pname, "z_award_boss_fight")
  elseif en == "zombies:cyclops"      then ZA_unlock(pname, "z_award_one_eyed_menace")
  elseif en == "zombies:megaxol"      then ZA_unlock(pname, "z_award_jaxolaughter")
  elseif en == "zombies:snowgolem"    then ZA_unlock(pname, "z_award_ho_ho_horror")
  elseif en == "zombies:gnoll"        then ZA_unlock(pname, "z_award_knavish_gnolls")
  elseif en == "zombies:froghman" or en == "zombies:frarcher"
                                      then ZA_unlock(pname, "z_award_mortal_enemy")
  elseif en == "default:mosasaurus" or en:lower():find("mosasaur")
                                      then ZA_unlock(pname, "z_award_leviathan_slayer")
  end

  if BASE_ZOMBIES[en] then
    if any_tex_matches(captured_textures, {"scarecrow1","scarecrow2"}) then ZA_unlock(pname, "z_award_wrong_field") end
    if any_tex_matches(captured_textures, {"bacon","uncookedbacon"})   then ZA_unlock(pname, "z_award_baconator")   end
    if any_tex_matches(captured_textures, {"gasguy%.png"})             then ZA_unlock(pname, "z_award_apocalypse_when") end
  end
end

-- === Kill detection: wrap on_punch; notify triggers; unlock one-offs ===
local function hook_kill_detection()
  for name, def in pairs(minetest.registered_entities) do
    if name:match("^zombies:") or name:lower():find("mosasaur") then
      local ent_name = name
      local old_punch = def.on_punch

      def.on_punch = function(self, hitter, tf, toolcaps, dir, damage)
        -- Capture textures BEFORE damage resolves
        local captured_textures = entity_textures_from(self, def)

        -- Run original logic
        if old_punch then
          old_punch(self, hitter, tf, toolcaps, dir, damage)
        end

        -- After engine/frame processes death (guard against removed objects)
        -- After engine/frame processes death (guard against removed objects)
        local obj = self.object
        minetest.after(0, function(o, h, en, tex, luae)
          -- Determine if the mob died (robust to removal timing)
          local dead = false
          if luae and type(luae.health) == "number" then
            dead = (luae.health <= 0)
          end
          if not dead then
            local ok_pos, pos = pcall(function() return o and o:get_pos() end)
            if (not ok_pos) or (pos == nil) then dead = true end
          end
          if not dead then
            local ok_hp, hp = pcall(function() return o and o:get_hp() end)
            if (not ok_hp) or (hp == nil) then dead = true else dead = (hp <= 0) end
          end
          if not dead then return end

          -- Use PlayerRef for triggers; name only for unlock()
          local pl = (h and h.is_player and h:is_player()) and h or nil
          if not pl then return end
          local pname = pl:get_player_name()

          -- === Progress notifications for counted awards (PlayerRef required) ===
          if SLAYER_SET[en]      then notify_anyzombie(pl) end
          if en == "zombies:1arm"   then notify_onearm(pl)  end
          if en == "zombies:hamster" then notify_hamster(pl) end
          if en == "zombies:cyclops" then notify_cyclops(pl) end
          if JUNGLE_SET[en]         then notify_jungle(pl)   end

          local is_boss = (en == "zombies:badguy" or en == "zombies:badguys")
          if is_boss then
            local function has(p) return any_tex_matches(tex, {p}) end
            if has("bg1%.png")                    then notify_bg1(pl)  end
            if has("bg4%.png") or has("bg8%.png") then notify_troll(pl) end
            if has("bg5%.png")                    then notify_bg5(pl)  end
            if has("bg9%.png")                    then notify_bg9(pl)  end
          end

          -- One-off unlocks (string name is fine here)
          unlock_one_offs(pname, en, tex)

          -- Optional: horde survivor tally
          local ZA = rawget(_G, "zombies_awards")
          if ZA and ZA.horde and ZA.horde.active then
            ZA.horde.kills_by_player = ZA.horde.kills_by_player or {}
            ZA.horde.kills_by_player[pname] = (ZA.horde.kills_by_player[pname] or 0) + 1
          end
        end, obj, hitter, ent_name, captured_textures, self)
      end
    end
  end
end

minetest.register_on_mods_loaded(hook_kill_detection)


awards.register_award("beds:sleep_day", {
	title       = "Sleep the Day Away",
	description = "Use a bed or sleeping bag during daytime to turn it into night.",
	difficulty  = 1,
	-- icon     = "awards_bed.png",   -- optional
	-- prizes   = {"default:torch 8"}, -- optional
})



--------------------------------------------------------------------------
-- 5) Death-by-mob awards (hamster, gnoll, froghman/frarcher)
--------------------------------------------------------------------------

minetest.register_on_dieplayer(function(player, reason)
  if not (reason and reason.type == "punch" and reason.object) then
    return
  end
  local luae = reason.object.get_luaentity and reason.object:get_luaentity()
  if not (luae and luae.name) then return end
  local pname = player:get_player_name()

  if luae.name == "zombies:hamster" then
    unlock(pname, "z_award_hamster_fail")
  end
  if luae.name == "zombies:gnoll" then
    unlock(pname, "z_award_knavish_gnolls")
  end
  if luae.name == "zombies:froghman" or luae.name == "zombies:frarcher" then
    unlock(pname, "z_award_mortal_enemy")
  end

  -- Boss skin death awards
  if luae.name == "zombies:badguy" or luae.name == "zombies:badguys" then
    local self = luae
    local def = minetest.registered_entities[luae.name]
    local tex = entity_textures(self, def)
    local function has(pat) return any_tex_matches(tex, {pat}) end
    if has("bg1%.png") then awards.unlock(pname, "z_award_savage_conan") end
    if has("bg7%.png") then awards.unlock(pname, "z_award_oni_oh_no") end
  end


  -- Survivor disqualifier during active horde
  local ZA = rawget(_G, "zombies_awards")
  if ZA and ZA.horde and ZA.horde.active then
    ZA.horde.died[pname] = true
  end
end) -- Player death reasons API provides puncher object. :contentReference[oaicite:2]{index=2}

--------------------------------------------------------------------------
-- 6) “Meet” awards: proximity (bigape, buffpig, parrot) – light, throttled
--------------------------------------------------------------------------

local function proximity_awarder(ent_name, award, radius)
  local def = minetest.registered_entities[ent_name]
  if not def then return end
  local old_step = def.on_step
  def.on_step = function(self, dtime)
    self._za_t = (self._za_t or 0) + dtime
    if self._za_t >= 1.0 then
      self._za_t = 0
      local pos = self.object:get_pos()
      ...
      if nearest then
        unlock(nearest:get_player_name(), award)
      end
    end
...
proximity_awarder("zombies:bigape",  "z_award_bigfoot",        16)
proximity_awarder("zombies:buffpig", "z_award_i_could_never",  12)
proximity_awarder("zombies:parrot",  "z_award_seaworthy_mate", 12)

--------------------------------------------------------------------------
-- 7) Golemancer: simple API to call from where you spawn golems
--------------------------------------------------------------------------

zombies_awards = rawget(_G, "zombies_awards") or {}
function zombies_awards.on_spawn_golem(playername, golem_name)
  if playername and golem_name and golem_name:find("^zombies:.+_golem$") then
    unlock(playername, "z_award_golemancer")
  end
end

--------------------------------------------------------------------------
-- 8) Survivor: tiny handshake with your horde system
--    Call zombies_awards.begin_horde(total_spawned) at the start
--    Call zombies_awards.end_horde() after the final wave ends
--------------------------------------------------------------------------

zombies_awards.horde = zombies_awards.horde or {
  active = false, total = 0, kills_by_player = {}, died = {}
}

function zombies_awards.begin_horde(total_spawned)
  local H = zombies_awards.horde
  H.active = true
  H.total  = tonumber(total_spawned) or 0
  H.kills_by_player = {}
  H.died = {}
end

function zombies_awards.end_horde()
  local H = zombies_awards.horde
  if not H.active then return end
  for pname, kills in pairs(H.kills_by_player) do
    if not H.died[pname] and kills >= H.total and H.total > 0 then
      unlock(pname, "z_award_survivor")
    end
  end
  H.active = false
  H.total = 0
