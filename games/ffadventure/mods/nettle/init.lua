...
minetest.register_abm({
	nodenames = {"group:soft_rock", "group:soft_stone", "group:stone", "default:limestone", "default:granite", "default:obsidian", "default:saprolite", "default:rhyolite"},
	interval = 240,
	chance = 2048, -- Adjusted for a 1% chance (100% / 10000 = 0.01%)
	action = function(pos, node)
		local above = {x=pos.x, y=pos.y+1, z=pos.z}
		if minetest.get_node(above).name ~= "air" then
			return
		...
		for x_ = pos.x-1, pos.x+1 do
			for y_ = pos.y-1, pos.y+1 do
				for z_ = pos.z-1, pos.z+1 do
					local nn = minetest.get_node({x=x_, y=y_, z=z_}).name
					if (nn == "default:granite") or nn == ("default:stone") then
						dirts = dirts+1;
					end
					if (nn == "air") or (minetest.get_item_group(nn, "flora") > 0) then
						airs = airs+1;
		...
		if (minetest.get_node({x=pos.x, y=pos.y+2, z=pos.z}).name ~= "air") or (minetest.get_node_light(above, 0.5) < 8) then
			minetest.set_node(above, {name = "rockweed:seed"})
		elseif minetest.get_node_light(above, 0.5) < 13 then
			minetest.set_node(above, {name = "rockweed:seed"})
		else
			minetest.set_node(above, {name = "rockweed:seed"})
...
minetest.register_abm({
	nodenames = {"group:spreading_dirt_type", "default:saprolite", "default:dirt_with_rainforest_litter", "default:rhyolite"},
	interval = 240,
	chance = 2048, -- Adjusted for a 1% chance (100% / 10000 = 0.01%)
	action = function(pos, node)
		local above = {x=pos.x, y=pos.y+1, z=pos.z}
		...
		if (minetest.get_node({x=pos.x, y=pos.y+2, z=pos.z}).name ~= "air") or (minetest.get_node_light(above, 0.5) < 8) then
			minetest.set_node(above, {name = "mykota:mykota"})
		elseif minetest.get_node_light(above, 0.5) < 13 then
			minetest.set_node(above, {name = "mykota:mykota"})
		else
			minetest.set_node(above, {name = "mykota:mykota"})
...
minetest.register_abm({
	...
	action = function(pos, node)
		...
		if (minetest.get_node({x=pos.x, y=pos.y+2, z=pos.z}).name ~= "air") or (minetest.get_node_light(above, 0.5) < 8) then
			minetest.set_node(above, {name = "kudzu:kudzu"})
		elseif minetest.get_node_light(above, 0.5) < 13 then
			minetest.set_node(above, {name = "kudzu:kudzu"})
		else
			minetest.set_node(above, {name = "kudzu:kudzu"})
