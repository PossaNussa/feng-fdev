...
local function allow_metadata_inventory_put (pos, listname, index, stack, player)
	...
	if listname == "bucket_out" then
		if stack:get_name() ~= "bucket:bucket_empty_uni[_%a]*$" and not fluidity.florbs.get_is_florb(stack) then
			return 0
		end
...
local function melter_node_timer(pos, elapsed)
	...
	if inv:get_stack("heat", 1):get_name() == "bucket:bucket_lava_uni[_%a]*$" then
	...
	if (bucket_name == "bucket:bucket_empty_uni[_%a]*$" or fluidity.florbs.get_is_florb(bucket_out)) and metal.fluid ~= "" and bucket_out:get_count() == 1 then
...
if minetest.get_modpath("pipeworks") ~= nil then
	...
	local function insert_object(pos, node, stack, direction, owner)
		...
		elseif stack_name == "bucket:bucket_lava_uni[_%a]*$" then
