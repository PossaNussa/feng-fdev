...
function lottmobs.on_take(inv, listname, count, index, stack, player)
    if listname == "takeaway" then
        if lottmobs.check_pay(inv, true) then
            ...
            if thing == "moneybag:bolsa_dinero_1" or thing == "moneybag:bolsa_dinero_2" then
                -- Coins are already deducted in check_pay
            else
...
function lottmobs.check_pay(inv, paynow)
    local now_at_pay = inv:get_stack("payment", 1)
    if now_at_pay:is_empty() then
        return false
    end

    local count = now_at_pay:get_count()
    local name = now_at_pay:get_name()

    local price_stack = inv:get_stack("price", 1)
    local price_count = price_stack:get_count()
    local price_name = price_stack:get_name()

    if price_name == name then
        if price_count > 0 and price_count <= count then
            if paynow then
                local change = count - price_count
                -- Ensure change is non-negative
                change = math.max(change, 0)
                now_at_pay:take_item(price_count)  -- Use take_item to directly handle the count
                
                if change > 0 then
                    -- Create a new bag of gold with the remaining coins
                    local new_stack = ItemStack("moneybag:bolsa_dinero_1")
                    local meta = new_stack:get_meta()
                    meta:set_int("coins", change)
                    inv:set_stack("payment", 1, new_stack)
                else
                    inv:set_stack("payment", 1, nil)
                end
                return true
            else
                return true
    ...
    -- Handle the case where the payment is a money bag
    elseif name == "moneybag:bolsa_dinero_1" or name == "moneybag:bolsa_dinero_2" then
        local meta = now_at_pay:get_meta()
        if not meta then
            minetest.log("error", "Meta is nil for item: " .. name)
            return false
        end

        local coins_in_bag = meta:get_int("coins") or 0

        if coins_in_bag >= price_count then
            if paynow then
                -- Deduct the required amount of coins from the money bag
                coins_in_bag = coins_in_bag - price_count
                meta:set_int("coins", coins_in_bag)

                -- Update inventory image based on remaining coins
                if coins_in_bag == 0 then
                    now_at_pay:set_name("moneybag:bolsa_dinero_2")
                else
                    now_at_pay:set_name("moneybag:bolsa_dinero_1")
                end

                inv:set_stack("payment", 1, now_at_pay)
                return true
            else
                return true
            end
...
mobs:register_mob("lottmobs:mung_trader", {
	type = "npc",
	race = "mung",
	hp_min = 15,
	hp_max = 35,
	collisionbox = {-0.3, -0.01, -0.3, 0.3, 1.94, 0.3},  -- Using the original collision box from mobs_mc:villager
	textures = {
	...
	light_damage = 0,
	attack_type = "dogfight",
	follow = {"default:apple", "default:bread", "farming:bread"},
	animation = {
	...
	step = 1,
	on_rightclick = function(self, clicker)
		lottmobs_trader(self, clicker, entity, lottmobs.mung_trader, "gui_gondorbg.png", "mung")
	end,
})

  mobs:register_spawn("lottmobs:mung_trader", {"default:sand", "default:dirt_with_coniferous_litter", "default:dirt_with_grass", "default:cobble", "default:obsidian", "default:onyx", "default:slate", "default:gravel", "default:saprolite", "default:limestone", "default:sandstone"}, 15, 0, 24000, 3, 31000, false)
minetest.register_alias("mobs_mc:villager", "lottmobs:mung_trader")

mobs:register_egg("lottmobs:mung_trader", ("Mung"), "#563d33", "#bc8b72", 0)
