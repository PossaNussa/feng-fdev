...
public:
	 ...
	 *
	 * To reduce network traffic the server does not immediately send
	 * inventory updates on every minor change. Instead the server
	 * accumulates time after the inventory becomes dirty and sends
	 * updates at most once per `inventory_update_interval` seconds.
	 *
	 * - `m_inv_update_interval` stores the configured interval (seconds).
	 *   It is loaded from settings on the first RemotePlayer construction.
	 * - `m_inv_setting_cache_loaded` indicates whether the static
	 *   configuration has been loaded.
	 * - `m_inv_send_timer` accumulates elapsed time since the last
	 *   inventory send while the inventory is dirty. Once it exceeds
	 *   `m_inv_update_interval` the server will push an update and reset
	 *   the timer.
	 */
	static bool m_inv_setting_cache_loaded;
	static float m_inv_update_interval;
	float m_inv_send_timer = 0.0f;
