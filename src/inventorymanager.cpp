...
void IMoveAction::apply(InventoryManager *mgr, ServerActiveObject *player, IGameDef *gamedef)
{
	...
	*/
    ...
    // Determine whether this move is a trivial reorder within the same
    // player inventory and list.  Such moves do not affect game logic
    // (items remain in the same inventory and list) and can be handled
    // entirely on the client without immediate server notification.
	...
	*/
	...
	if (from_inv == to_inv) {
        ...
        // Notify server of inventory changes unless this is a trivial
        // reorder within the same player inventory and list.
        if (!trivial_move)
...
void IMoveAction::clientApply(InventoryManager *mgr, IGameDef *gamedef)
{
	...
	// Local reordering within the same player inventory/list does not need
	// to mark the inventory as modified; such changes are purely visual.
	bool trivial_move = false;
	if (from_inv.type == InventoryLocation::PLAYER &&
	    to_inv.type   == InventoryLocation::PLAYER &&
	    from_inv == to_inv &&
	...
	if (!trivial_move)
		mgr->setInventoryModified(from_inv);
	if (!trivial_move && inv_from != inv_to)
		mgr->setInventoryModified(to_inv);
}
