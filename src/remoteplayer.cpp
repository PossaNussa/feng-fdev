...
// Static initialisation for inventory update settings.  The default
// interval is 1.0 second; it will be overwritten with the value of
// `inventory_update_interval` from settings on the first RemotePlayer
// construction.
bool RemotePlayer::m_inv_setting_cache_loaded = false;
float RemotePlayer::m_inv_update_interval = 1.0f;
	...
	Player(name, idef)
{
    ...
    // Load inventory update interval setting on first RemotePlayer construction.
    if (!RemotePlayer::m_inv_setting_cache_loaded) {
        if (g_settings->exists("inventory_update_interval"))
            RemotePlayer::m_inv_update_interval =
                g_settings->getFloat("inventory_update_interval");
        else
            RemotePlayer::m_inv_update_interval = 1.0f;
        RemotePlayer::m_inv_setting_cache_loaded = true;
    }
    // Initialise per-instance timer for inventory syncing
    m_inv_send_timer = 0.0f;
