...
struct MeshMakeData
{
	...
	/**
	 * Whether this mesh should use a simplified level‑of‑detail (LOD) when
	 * generating geometry. When set to true the mesh generator will skip
	 * decorative drawtypes (plantlike, signlike, firelike, etc.) and draw
	 * only the top faces of solid/liquid nodes. This significantly reduces
	 * the number of vertices generated for far away mapblocks at the cost
	 * of visual fidelity. The value is set by MapBlockMesh based on the
	 * distance to the camera and the configured LOD distance. Mods and
	 * rendering code should treat this as read‑only.
	 */
	bool m_far_away = false;
