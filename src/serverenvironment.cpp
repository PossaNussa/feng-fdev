...
void ServerEnvironment::step(float dtime)
{
	...
	*/
    ...
    // Send outdated player inventories.  To reduce network traffic we
    // throttle inventory synchronization.  Each RemotePlayer accumulates
    // elapsed time since their inventory became dirty; only when the
    // accumulation exceeds the configured interval will the server send
    // the updated inventory.  See RemotePlayer::m_inv_update_interval.
    for (RemotePlayer *player : m_players) {
        if (player->getPeerId() == PEER_ID_INEXISTENT)
            continue;
        if (player->inventory.checkModified()) {
            // accumulate time
            player->m_inv_send_timer += dtime;
            if (player->m_inv_send_timer >= RemotePlayer::m_inv_update_interval) {
                m_server->SendInventory(player, true);
                // reset timer after sending
                player->m_inv_send_timer = 0.0f;
            }
        } else {
            // reset timer when inventory no longer dirty
            player->m_inv_send_timer = 0.0f;
        }
    }
